...
- `Use` tab: model source selection shell, active route card, nested `Route / Mount / Local / Private / Public / Cloud` sections
- `Mount` now owns the OpenClaw integration/config surface inside `Use`, while `Route` focuses on route health and mounted-edge status
